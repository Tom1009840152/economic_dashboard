...
- `backend/app/services/release_evidence.py`（独立、只追加的首发证据版本链）
...
官方若有后续不同版本则继续追加而不覆盖。A3 `1.2.0` 对相同指标和观察期优先读取这条证据链，
避免修订后的最终值倒灌历史。当前 `CN_GDP` 展示用镜像的来源链接已纠正为
...
写入前后 `data_points` 均为 331 条且内容指纹相同。固定 `end=2026-07` 的 A3 回放由
