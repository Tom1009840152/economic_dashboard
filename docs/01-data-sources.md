...
D9 新增独立、只追加的 `release_evidence`：从国家统计局 `stats.gov.cn` 和财政部
`gks.mof.gov.cn` 原发布稿恢复首发值、来源链接与真实发布时间；同一证据重复运行不新增，
官方若有后续不同版本则继续追加而不覆盖。A3 `1.3.1` 只有在 `chain_verified=true`、证据类型
和时间精度都受支持时才让这条证据链优先于通用版本，避免修订后的最终值倒灌历史，也避免
任意入表记录取得权威地位。当前 `CN_GDP` 展示用镜像的来源链接已纠正为
...
这是当次部署验收记录，不是当前全库总行数。固定 `end=2026-07` 的 A3 回放由
1,831 条可还原、1,819 条按时可用、16/23 个代码，提升为 1,846、1,822、17/23。
...
384 条后 `release_evidence` 总量为 678 条；相同参数复跑三项均新增 0 条。完整性门要求三序列
...
固定 `end=2026-07` 回放升级后，可还原输入从财政里程碑时的 1,846 条增至 1,973 条，按各信号
模型目标月可用 1,896 条，具备严格历史的代码从 17/23 增至 18/23。2026-07 使用 2026-06
