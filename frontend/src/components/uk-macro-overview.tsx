...
import type {
  UKMacroConfidence,
  UKMacroMetric,
  UKMacroOverviewDashboard,
  UKMacroPillar,
  UKMacroTone,
} from "@/lib/api";

const TONE_STYLES: Record<UKMacroTone, string> = {
  positive:
...
> = {
  growth: {
    icon: Activity,
    href: "/country/uk/growth",
    linkLabel: "查看英国增长原始指标",
  },
  labour: {
  ...
  inflation: {
    icon: Gauge,
    href: "/country/uk/prices",
    linkLabel: "查看英国通胀原始指标",
  },
  financial_conditions: {
    icon: ChartCandlestick,
    href: "/country/uk/markets",
    linkLabel: "查看市场与外部指标",
  },
  monetary_policy: {
    icon: Landmark,
    href: "/country/uk/monetary",
    linkLabel: "查看Bank Rate与货币指标",
  },
};

const CONFIDENCE_LABELS: Record<UKMacroConfidence, string> = {
  high: "高覆盖",
  medium: "中等置信",
  low: "低覆盖",
  unavailable: "不可判断",
...
function formatValue(metric: UKMacroMetric) {
...
function formatReference(metric: UKMacroMetric) {
...
function TrendIcon({ metric }: { metric: UKMacroMetric }) {
  if (metric.trend === "up") {
...
function MetricCell({ metric }: { metric: UKMacroMetric }) {
...
function PillarCard({ pillar }: { pillar: UKMacroPillar }) {
  ...
  return (
    <Card className="border-amber-300 dark:border-amber-900">
      <CardContent className="flex items-start gap-3 p-5">
        ...
        <div>
          <h2 className="font-semibold">英国综合分析暂不可用</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            分析接口本次加载失败。失败不表示英国经济处于中性，也不表示相关指标为零；可先查看各分类原始指标。
          </p>
          <Link
            href="/country/uk/growth"
...
export function UKMacroOverview({
  data,
  failed,
}: {
  data: UKMacroOverviewDashboard | null;
  failed: boolean;
}) {
  ...
  return (
    <div className="space-y-5">
        ...
        <CardContent className="space-y-4 p-4 sm:p-5">
          <div className="grid gap-3 text-xs leading-5 text-muted-foreground md:grid-cols-2">
            <div className="rounded-xl border bg-muted/20 p-3.5">
              ...
              本页观察整个英国经济，采用 ONS 与英格兰银行等英国口径；英国不属于欧元区，政策利率是 Bank Rate，而不是 ECB 利率。
            </div>
            <div className="rounded-xl border bg-muted/20 p-3.5">
              <span className="font-medium text-foreground">覆盖边界：</span>
              金融条件仅使用当前可得的广义货币和市场代理，尚缺完整英债曲线、公司信用利差、按揭与银行贷款条件。
            </div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
            ...
            <div className="rounded-xl border bg-muted/25 p-4">
              ...
              <p className="mt-3 border-t pt-3 text-xs leading-5 text-muted-foreground">
                ONS、英格兰银行与市场数据发布节奏不同，因此保留各自截止日期，不强行拼成同一天的“当前值”。
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">英格兰银行政策到实体经济：当前传导链</CardTitle>
          <p className="text-sm text-muted-foreground">
            用 Bank Rate、货币与市场代理及实体指标组织证据；相关性不等于确定因果。
          </p>
        </CardHeader>
        <CardContent>
