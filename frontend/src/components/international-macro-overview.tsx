...
type RegionSlug = "jp" | "kr";

export interface InternationalMacroOverviewData {
  ...
  downturn_breadth: USRecessionBreadth;
...
export interface InternationalMacroOverviewConfig {
...
function formatValue(metric: USMacroMetric) {
  ...
  return withUnit(value, metric.unit);
}
...
function formatReference(metric: USMacroMetric) {
  ...
  return `参考 ${metric.reference_period}：${withUnit(value, metric.unit)}`;
}
    ...
    <div className="rounded-xl border bg-background/80 p-3.5">
      ...
      <details className="mt-2 text-[11px] text-muted-foreground">
        ...
        <div className="mt-1.5 space-y-1 border-l pl-2.5">
          ...
          <div>输入：{metric.source_codes.join("、") || "当前无可用输入"}</div>
        </div>
      </details>
    </div>
  );
...
function pillarPresentation(
  pillar: USMacroPillar,
  config: InternationalMacroOverviewConfig,
): { icon: LucideIcon; href: string; linkLabel: string } {
  const base = `/country/${config.regionSlug}`;
  ...
  > = {
    growth: {
      icon: Activity,
      href: `${base}/growth`,
      linkLabel: `查看${config.countryLabel}增长原始指标`,
    },
    labour: {
      icon: BriefcaseBusiness,
      href: `/employment/${config.regionSlug}`,
      linkLabel: `查看${config.countryLabel}就业专题`,
    },
    inflation: {
      icon: Gauge,
      href: `${base}/prices`,
      linkLabel: `查看${config.countryLabel}通胀原始指标`,
    },
    financial_conditions: {
      icon: ChartCandlestick,
      href: `${base}/markets`,
      linkLabel: config.marketLinkLabel,
    },
    monetary_policy: {
      icon: Landmark,
      href: `${base}/monetary`,
      linkLabel: config.policyLinkLabel,
    },
  };
  return presentations[pillar.key];
}
...
function PillarCard({
  ...
  const meta = pillarPresentation(pillar, config);
...
function UnavailableOverview({ config }: { config: InternationalMacroOverviewConfig }) {
  return (
    <Card className="border-amber-300 dark:border-amber-900">
      <CardContent className="flex items-start gap-3 p-5">
        <AlertTriangle className="mt-0.5 size-5 shrink-0 text-amber-700 dark:text-amber-300" />
        <div>
          <h2 className="font-semibold">{config.countryLabel}综合分析暂不可用</h2>
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            分析接口本次加载失败。失败不表示经济处于中性，也不表示相关指标为零；可先查看各分类原始指标。
          </p>
          <Link
            href={`/country/${config.regionSlug}/growth`}
            className="mt-3 inline-flex items-center gap-1 text-sm font-medium hover:underline"
          >
            查看{config.countryLabel}原始数据
            <ChevronRight className="size-4" aria-hidden="true" />
          </Link>
        </div>
      </CardContent>
...
export function InternationalMacroOverview({
  data,
  failed,
  config,
}: {
  data: InternationalMacroOverviewData | null;
  failed: boolean;
  config: InternationalMacroOverviewConfig;
}) {
  if (failed || data == null) return <UnavailableOverview config={config} />;
  ...
  return (
    <div className="space-y-5">
      <Card className="overflow-hidden border-foreground/25 shadow-sm">
        <CardHeader className="gap-5 bg-foreground text-background lg:flex lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 max-w-4xl flex-1">
            ...
            <p className="mt-3 text-sm leading-6 opacity-75">{config.introduction}</p>
          </div>
      ...
      <div className="columns-1 gap-5 xl:columns-2">
        {data.pillars.map((pillar) => (
          <div key={pillar.key} className="mb-5 inline-block w-full break-inside-avoid">
            <PillarCard pillar={pillar} config={config} />
          </div>
        ))}
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">{config.transmissionTitle}</CardTitle>
          <p className="text-sm text-muted-foreground">{config.transmissionDescription}</p>
        </CardHeader>
        <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <ul className="space-y-3 text-sm leading-6 text-muted-foreground">
              {data.watch_items.length > 0 ? data.watch_items.map((item) => (
          ...
          <CardContent className="space-y-4 text-sm leading-6 text-muted-foreground">
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3.5 text-amber-950 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
              ...
              <ul className="mt-2 space-y-1.5 text-xs leading-5">
                {data.warnings.length > 0
                  ...
                  : <li>· 暂无附加警告，仍需结合各指标观察期与方法说明理解。</li>}
              </ul>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-4 text-xs">
              ...
              >
                ...
                查看{config.countryLabel}数据来源与更新
              </Link>
            </div>
          </CardContent>
        </Card>
